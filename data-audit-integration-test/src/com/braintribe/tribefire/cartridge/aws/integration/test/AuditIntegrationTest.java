...
public class AuditIntegrationTest extends AbstractTribefireQaTest {
	...
	public static void initialize() throws Exception {
		...
		GmMetaModel configuredUserModel = imp.model()
				.create(DATA_AUDIT_TEST_CONFIGURATION_MODEL + "-" + uuid, "com.braintribe.gm:user-model", "com.braintribe.gm:query-model").get();
		...
		CollaborativeSmoodAccess auditAccess = session.create(CollaborativeSmoodAccess.T);
		...
		GmMetaModel configuredUserAndAuditModel = imp.model().create(DATA_AUDIT_TEST_COMBINED_CONFIGURATION_MODEL + "-" + uuid,
				"com.braintribe.gm:user-model", "com.braintribe.gm:query-model", "tribefire.extension.audit:data-audit-model").get();
		...
		CollaborativeSmoodAccess combinedAccess = session.create(CollaborativeSmoodAccess.T);
		...
		CollaborativeSmoodAccess untrackedAccess = session.create(CollaborativeSmoodAccess.T);
	...
	public void testCollectionsAndNonCollections() throws Exception {
		...
		validator.add().kind(ManipulationType.added).type(User.T).property(User.roles).previousValue("(com.braintribe.m...")
	...
	public void testPreviousCollection() throws Exception {
		...
		validator.add().kind(ManipulationType.propertyChanged).type(SelectQueryResult.T).property("results").previousValue("[1,2,3]")
				.value("[3,4,5]");
